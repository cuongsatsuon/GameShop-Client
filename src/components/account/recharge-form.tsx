...
export function RechargeForm() {
  ...
  return (
    <form onSubmit={submit} className="surface overflow-hidden p-5 sm:p-7">
      <div className="flex items-center justify-between gap-3 border-b border-border pb-5">
        <div>
          ...
          <h3 className="mt-2 text-xl font-extrabold text-white">Thêm số dư vào ví</h3>
        </div>
      ...
      <p className="mt-5 flex items-start gap-2 rounded-xl border border-cyan-300/15 bg-cyan-300/5 px-3 py-2.5 text-xs leading-5 text-muted-foreground"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-300" />Chọn đúng loại thẻ và mệnh giá trước khi gửi. Thẻ khai sai thông tin có thể xử lý chậm.</p>
      {done && <p className="mt-3 rounded-xl bg-success/10 px-3 py-2 text-sm text-success">{done}</p>}
      {error && <p className="mt-3 rounded-xl bg-destructive/10 px-3 py-2 text-sm text-destructive">{error}</p>}
      <Button type="submit" variant="gradient" size="lg" className="mt-5 w-full rounded-xl" disabled={loading}><Zap className="h-4 w-4" />{loading ? "Đang xử lý..." : `Nạp thẻ ngay · ${formatVND(amount)}`}</Button>
    </form>
  );
}
